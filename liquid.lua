--[[
    KEYSER UI V6.5 - OPTIMIZED & CORRECTED by AI
    Structure: Intro -> Sidebar (Main) -> Topbar (Sub) -> Content
    
    New Features & Updates:
    - Added Intro/Loading Sequence (Dex Style).
    - Added ColorPicker (Fatality Logic - HSV, Draggable).
    - Added Option System (Fatality Style - Gear Icon for Sub-Elements).
    - Refactored Element Builder for recursive nested elements (Options).
    - Kept all previous memory leak fixes and list filtering.
]]
...
local Keyser = {
    Colors = {
        ...
        Accent      = Color3.fromRGB(61, 143, 220), -- Blue accent for loading and primary highlights
...
function Library:Window(Config)
    ...
    local IntroFrame = Create("Frame", {
        Name = "Intro", Parent = Screen, BackgroundColor3 = Color3.fromRGB(45, 45, 45),
    ...
    local IntroHolder = Create("Frame", {Parent = IntroFrame, BackgroundColor3 = Color3.fromRGB(40, 40, 40), Size = UDim2.new(1, 0, 1, 0), ClipsDescendants = true, BorderSizePixel = 0})
    ...
    local IntroTitle = Create("TextLabel", {Parent = IntroHolder, BackgroundTransparency = 1, Position = UDim2.new(0, 20, 0, 15), Size = UDim2.new(1, -40, 0, 50), Font = Enum.Font.GothamBlack, Text = WindowName, TextColor3 = Color3.new(1,1,1), TextSize = 40, TextXAlignment = Enum.TextXAlignment.Left, TextTransparency = 1})
    local IntroDesc = Create("TextLabel", {Parent = IntroHolder, BackgroundTransparency = 1, Position = UDim2.new(0, 20, 0, 60), Size = UDim2.new(1, -40, 0, 25), Font = Keyser.Font, Text = "Ultimate Debugging Suite", TextColor3 = Color3.new(1,1,1), TextSize = 16, TextXAlignment = Enum.TextXAlignment.Left, TextTransparency = 1})
    ...
    local ProgressBarBg = Create("Frame", {Parent = IntroHolder, BackgroundColor3 = Color3.fromRGB(20, 20, 20), Position = UDim2.new(0, 20, 0, 145), Size = UDim2.new(1, -40, 0, 4), BorderSizePixel = 0})
    ...
    local IntroCreator = Create("TextLabel", {Parent = IntroFrame, BackgroundTransparency = 1, Position = UDim2.new(1, -120, 1, -25), Size = UDim2.new(0, 105, 0, 20), Font = Keyser.Font, Text = "Keyser Lib V6.5", TextColor3 = Color3.new(1,1,1), TextSize = 12, TextXAlignment = Enum.TextXAlignment.Right, TextTransparency = 1})

    -- Animação da Intro
    Tween(IntroTitle, {TextTransparency = 0}, 0.5)
    Tween(IntroDesc, {TextTransparency = 0}, 0.5)
    Tween(StatusText, {TextTransparency = 0}, 0.5)
    Tween(IntroCreator, {TextTransparency = 0}, 0.5)
    
    task.wait(0.5)
    
    local loadingSteps = {
        {0.2, "Bypassing Anticheat..."},
        {0.5, "Loading Assets..."},
        {0.8, "Building User Interface..."},
        {1.0, "Ready!"}
    }
    ...
    local MainFrame = Create("Frame", {
        ...
        BackgroundTransparency = 1 -- Inicia invisível para fade in
    })
    ...
    local Sidebar = Create("Frame", {Parent = MainFrame, BackgroundColor3 = Keyser.Colors.Sidebar, Position = UDim2.new(0, 0, 0, 65), Size = UDim2.new(0, 200, 1, -65), BorderSizePixel = 0})
    local LogoArea = Create("Frame", {Parent = Header, BackgroundTransparency = 1, Size = UDim2.new(0, 200, 1, 0)})
    Create("TextLabel", {Parent = LogoArea, BackgroundTransparency = 1, Position = UDim2.new(0, 0, 0, 14), Size = UDim2.new(1, 0, 0, 22), Font = Enum.Font.GothamBlack, Text = WindowName, TextColor3 = Color3.fromRGB(110, 110, 120), TextSize = 22})
    ...
    -- Function to Handle the Floating "Option" Menus
    local function CreateOptionFlyout(AnchorButton)
        local Flyout = Create("Frame", {
            Parent = Screen, BackgroundColor3 = Keyser.Colors.Sidebar,
        ...
        local function ToggleFlyout(state)
            ...
            if isOpen then
                ...
                Tween(Flyout, {Size = UDim2.new(0, 180, 0, targetHeight)}, 0.2)
                ...
                Tween(Flyout, {Size = UDim2.new(0, 180, 0, 0)}, 0.2)
                task.delay(0.2, function() if not isOpen then Flyout.Visible = false end end)
    ...
    -- Element Builder Factory (Used for Sections and Options)
    local function BuildElements(TargetParent)
        ...
        function Elements:Toggle(Cfg)
            ...
            local OptionBtn
            if Cfg.Option then
                OptionBtn = Create("ImageButton", {Parent = Frame, BackgroundTransparency = 1, Position = UDim2.new(1, -45, 0.5, -8), Size = UDim2.new(0, 16, 0, 16), Image = "rbxassetid://10734950309", ImageColor3 = Keyser.Colors.TextDark, ImageTransparency = 0.5})
            ...
            local function Update()
                if Toggled then 
                    Tween(CheckBg, {BackgroundColor3 = Keyser.Colors.AccentWhite})
            ...
            local ReturnAPI = {}
            if Cfg.Option then
                local FlyoutScroll = CreateOptionFlyout(OptionBtn)
                ReturnAPI.Option = BuildElements(FlyoutScroll)
            end
            return ReturnAPI
        end
        ...
        function Elements:Slider(Cfg)
            ...
            Trigger.InputBegan:Connect(function(i) 
                if i.UserInputType == Enum.UserInputType.MouseButton1 then 
                    local Dragging = true; 
                    local x = math.clamp((i.Position.X - Rail.AbsolutePosition.X)/Rail.AbsoluteSize.X,0,1); 
                    Set(Min + (Max-Min)*x); 
                    local changed, ended; 
                    changed = UserInputService.InputChanged:Connect(function(i2) 
                        if Dragging and i2.UserInputType == Enum.UserInputType.MouseMovement then 
                            local x2 = math.clamp((i2.Position.X - Rail.AbsolutePosition.X)/Rail.AbsoluteSize.X,0,1); 
                            Set(Min + (Max-Min)*x2) 
                        end 
                    end); 
                    ended = UserInputService.InputEnded:Connect(function(i3) 
                        if i3.UserInputType == Enum.UserInputType.MouseButton1 then 
                            Dragging = false; changed:Disconnect(); ended:Disconnect() 
                        end 
                    end) 
                end 
            end); Set(Val)
        ...
        function Elements:ColorPicker(Cfg)
            ...
            Create("UIStroke", {Parent = ColorBtn, Color = Keyser.Colors.Stroke})
            
            local Flyout = Create("Frame", {Parent = Screen, BackgroundColor3 = Keyser.Colors.Sidebar, Size = UDim2.new(0, 180, 0, 160), Position = UDim2.new(0,0,0,0), ClipsDescendants = true, ZIndex = 110, Visible = false})
            Create("UICorner", {Parent = Flyout, CornerRadius = UDim.new(0, 6)})
            Create("UIStroke", {Parent = Flyout, Color = Keyser.Colors.Stroke, Thickness = 1})

            local SatValMap = Create("ImageButton", {Parent = Flyout, Position = UDim2.new(0, 10, 0, 10), Size = UDim2.new(0, 140, 0, 110), BackgroundColor3 = ColorBtn.BackgroundColor3, AutoButtonColor = false})
            Create("UICorner", {Parent = SatValMap, CornerRadius = UDim.new(0, 4)})
            local SMapGradient = Create("UIGradient", {Parent = SatValMap, Color = ColorSequence.new(Color3.new(1,1,1), Color3.new(1,1,1)), Transparency = NumberSequence.new({NumberSequenceKeypoint.new(0,0), NumberSequenceKeypoint.new(1,1)})})
            local VMap = Create("Frame", {Parent = SatValMap, Size = UDim2.new(1,0,1,0), BackgroundColor3 = Color3.new(0,0,0)})
            Create("UICorner", {Parent = VMap, CornerRadius = UDim.new(0, 4)})
            local VMapGradient = Create("UIGradient", {Parent = VMap, Rotation = 90, Transparency = NumberSequence.new({NumberSequenceKeypoint.new(0,1), NumberSequenceKeypoint.new(1,0)})})
            ...
            local HueRail = Create("ImageButton", {Parent = Flyout, Position = UDim2.new(0, 155, 0, 10), Size = UDim2.new(0, 15, 0, 110), BackgroundColor3 = Color3.new(1,1,1), AutoButtonColor = false})
            Create("UICorner", {Parent = HueRail, CornerRadius = UDim.new(0, 4)})
            local HueGradient = Create("UIGradient", {Parent = HueRail, Rotation = 90, Color = ColorSequence.new({
            ...
            local Hue, Sat, Val = ColorBtn.BackgroundColor3:ToHSV()
            local updater, isOpen = nil, false

            local function UpdateColor()
                local newColor = Color3.fromHSV(Hue, Sat, Val)
                ColorBtn.BackgroundColor3 = newColor
                SatValMap.BackgroundColor3 = Color3.fromHSV(Hue, 1, 1)
                HexBox.Text = "#" .. newColor:ToHex():upper()
            ...
            local function ToggleFlyout(state)
                ...
                if state then
                    updater = RunService.RenderStepped:Connect(function()
                        Flyout.Position = UDim2.new(0, ColorBtn.AbsolutePosition.X - 150, 0, ColorBtn.AbsolutePosition.Y + 25)
                    end)
                else
                    if updater then updater:Disconnect(); updater = nil end
                end
                ...
                btn.InputBegan:Connect(function(input)
                    if input.UserInputType == Enum.UserInputType.MouseButton1 then
                        ...
                        local function upd(inp)
                            ...
                            if type == "Hue" then Hue = 1 - y; CursorH.Position = UDim2.new(0,0,y,0)
                            else 
                                local x = math.clamp((inp.Position.X - btn.AbsolutePosition.X)/btn.AbsoluteSize.X, 0, 1)
                                Sat = x; Val = 1 - y; CursorSV.Position = UDim2.new(x, -2, y, -2)
                            end
                            UpdateColor()
                        end
                        upd(input)
            ...
            HandleDrag(HueRail, "Hue")
            HandleDrag(SatValMap, "SV")
            
            UserInputService.InputBegan:Connect(function(input)
                if isOpen and input.UserInputType == Enum.UserInputType.MouseButton1 then
                    local mx, my = input.Position.X, input.Position.Y
                    local fx, fy = Flyout.AbsolutePosition.X, Flyout.AbsolutePosition.Y
                    local bx, by = ColorBtn.AbsolutePosition.X, ColorBtn.AbsolutePosition.Y
                    if not (mx >= fx and mx <= fx + Flyout.AbsoluteSize.X and my >= fy and my <= fy + Flyout.AbsoluteSize.Y) and not (mx >= bx and mx <= bx + ColorBtn.AbsoluteSize.X and my >= by and my <= by + ColorBtn.AbsoluteSize.Y) then
                        ToggleFlyout(false)
                    end
                end
            end)
    ...
    function WinData:Tab(Config)
        ...
        Create("UIListLayout", {Parent = TopButtonsFrame, FillDirection = Enum.FillDirection.Horizontal, HorizontalAlignment = Enum.HorizontalAlignment.Center, VerticalAlignment = Enum.VerticalAlignment.Center, Padding = UDim.new(0, 20)})

        local SideBtn = Create("TextButton", {Parent = SideContainer, BackgroundColor3 = Color3.fromRGB(30, 30, 35), BackgroundTransparency = 1, Size = UDim2.new(1, 0, 0, 38), Text = "", AutoButtonColor = false})
        ...
        local Label = Create("TextLabel", {Parent = SideBtn, BackgroundTransparency = 1, Position = UDim2.new(0, 40, 0, 0), Size = UDim2.new(1, -40, 1, 0), Font = Keyser.Font, Text = Config.Name, TextColor3 = Keyser.Colors.TextDark, TextSize = 13, TextXAlignment = Enum.TextXAlignment.Left})
        
        TabObj.TopTabs = {}
        TabObj.ActiveTop = nil
        ...
        function TabObj:Page(Name)
            local PageObj = {}
            local TopBtn = Create("TextButton", {Parent = TopButtonsFrame, BackgroundColor3 = Color3.fromRGB(30, 30, 35), BackgroundTransparency = 1, Size = UDim2.new(0, 0, 0, 32), Font = Keyser.FontBold, Text = Name, TextColor3 = Keyser.Colors.TextDark, TextSize = 13, AutomaticSize = Enum.AutomaticSize.X})
            ...
            local PageScroll = Create("ScrollingFrame", {Parent = PageContainer, BackgroundTransparency = 1, Size = UDim2.new(1, 0, 1, 0), Visible = false, ScrollBarThickness = 2, ScrollBarImageColor3 = Keyser.Colors.Stroke, CanvasSize = UDim2.new(0,0,0,0), BorderSizePixel = 0})
            ...
            PageObj.Btn = TopBtn; PageObj.Page = PageScroll
            ...
            function SectionLib:Section(SecConfig)
                ...
                local HeaderFrame = Create("Frame", {Parent = Groupbox, BackgroundColor3 = Keyser.Colors.Header, Size = UDim2.new(1, 0, 0, 40)})
                ...
                Create("Frame", {Parent = Groupbox, BackgroundColor3 = Keyser.Colors.Divider, BorderSizePixel = 0, Position = UDim2.new(0, 0, 0, 40), Size = UDim2.new(1, 0, 0, 1), ZIndex = 2})

                local ContentFrame = Create("Frame", {Parent = Groupbox, BackgroundColor3 = Keyser.Colors.Content, Position = UDim2.new(0, 0, 0, 41), Size = UDim2.new(1, 0, 0, 0), AutomaticSize = Enum.AutomaticSize.Y})
                ...
                Create("UIListLayout", {Parent = ContentFrame, SortOrder = Enum.SortOrder.LayoutOrder, Padding = UDim.new(0, 8)})
    ...
    task.defer(function()
        if not WinData.ActiveSidebar and #AllSidebarTabs > 0 then
            AllSidebarTabs[1]:Activate()
        end
    end)
